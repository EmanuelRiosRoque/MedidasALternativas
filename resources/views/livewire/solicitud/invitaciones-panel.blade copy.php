          ...
          <div class="relative mb-5" x-data="{ open: {{ $loop->first ? 'false' : 'true' }} }">
            ...
            <div class="rounded-lg border border-neutral-200 dark:border-neutral-700 p-4 bg-white dark:bg-neutral-900">
              ...
              <div x-show="open" x-collapse x-cloak>
                ...
                $facInvit = optional($inv->facilitador ?? null)->nombre ?? null;
                @endphp

                @if ($evForInv)
                <div
                  class="mt-3 rounded-lg border border-emerald-200 dark:border-emerald-700 bg-emerald-50/40 dark:bg-emerald-900/10 p-3">
                  ...
                  <div class="mt-2 text-sm">
                    ...
                    <div class="grid sm:grid-cols-2 gap-3">
                      <div class="rounded-md border border-emerald-200 dark:border-emerald-700 p-2">
                        ...
                        <p class="text-sm font-medium">{{ $hEvIni ? $fmtHora($hEvIni) : '—' }} - {{ $hEvFin ?
                          $fmtHora($hEvFin) : '—' }}</p>
                      </div>
                      <div class="rounded-md border border-emerald-200 dark:border-emerald-700 p-2">
                        <p class="text-[11px] text-neutral-500">Invitado(s)</p>
                        <p class="text-sm font-medium">{{ $hEvIniI ? $fmtHora($hEvIniI) : '—' }} - {{ $hEvFinI ?
                          $fmtHora($hEvFinI) : '—' }}</p>
                      </div>
                    </div>
                    @endif
                  ...
                  <div class="mt-2 text-xs">
                    <span
                      class="px-2 py-1 rounded-full bg-emerald-100 text-emerald-800 dark:bg-emerald-700/20 dark:text-emerald-300">
                ...
                <div
                  ...
                  <div class="flex items-center justify-between gap-2">
                    <p class="text-sm font-semibold text-neutral-800 dark:text-neutral-100">{{ $isPreMediacion ?
                      'Invitación' : 'Sesión' }}</p>
                    @if($inv->modalidad === 'linea')
                    <span class="text-xs text-neutral-600 dark:text-neutral-300">Fecha de atención: <span
                        class="font-medium">{{ $inv->fecha_atencion ?: $inv->fecha_envio ?: '—' }}</span></span>
                    @else
                    <span class="text-xs text-neutral-600 dark:text-neutral-300">Fecha de envío: <span
                        class="font-medium">{{ $inv->fecha_envio ?: $inv->fecha_atencion ?: '—' }}</span></span>
                    @endif
                  </div>
                  ...
                  <div class="mt-2 text-sm">
                    ...
                    <div class="rounded-md border border-neutral-200 dark:border-neutral-700 p-2 inline-block">
                      <p class="text-[11px] text-neutral-500">{{ $inv->modalidad === 'linea' ? 'Horario (invitación)' :
                    ...
                    <div class="grid sm:grid-cols-2 gap-3">
                      <div class="rounded-md border border-neutral-200 dark:border-neutral-700 p-2">
                        <p class="text-[11px] text-neutral-500">Solicitante(s)</p>
                        <p class="text-sm font-medium">{{ $inv->hora_inicio ? $fmtHora($inv->hora_inicio) : '—' }} - {{
                          $inv->hora_fin ? $fmtHora($inv->hora_fin) : '—' }}</p>
                      </div>
                      <div class="rounded-md border border-neutral-200 dark:border-neutral-700 p-2">
                        <p class="text-[11px] text-neutral-500">Invitado(s)</p>
                        <p class="text-sm font-medium">{{ $inv->hora_inicio_invitado ?
                          $fmtHora($inv->hora_inicio_invitado) : '—' }} - {{ $inv->hora_fin_invitado ?
                  ...
                  <div class="mt-2 text-xs">
                    ...
                    @if ($evForInv && $inv->facilitador && optional($evForInv->facilitador)->nombre !==
