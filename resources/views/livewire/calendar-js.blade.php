<div class="max-w-4xl mx-auto p-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-neutral-800 dark:text-white">
                    ...
                    <div class="flex-1 p-5">
                        ...
                        <div class="text-xs text-neutral-500 dark:text-neutral-300 flex flex-wrap gap-x-2">
                            @if ($evento->hora_inicio && $evento->hora_fin)
                                <span>
                                    @if (!$evento->opcion_invitacion) {{-- false = separados -> etiqueta solicitante --}}
    ...
    <div class="p-4 bg-neutral-200 dark:bg-neutral-600 rounded-lg shadow">
        <div class="flex items-center justify-between mb-2">
            <button wire:click="cambiarMes(-1)" class="text-neutral-600 dark:text-neutral-300 hover:text-black dark:hover:text-white">&lt;</button>
        ...
        <div class="mt-4 text-center">
            <x-flux::button wire:click="abrirModalDia" variant="primary" class="w-full hover:scale-105 transition-transform duration-150">
                Agregar evento
            </x-flux::button>
        </div>
    </div>
        ...
        <div class="space-y-6" x-data="{ mostrarObservacion: $wire.entangle('reasignacion') }">
            ...
            <div>
                <flux:heading size="lg">
                    {{ $modoEditar ? 'Actualizar el evento del día' : 'Crear evento para el día' }}: {{ $fechaSeleccionada }}
                </flux:heading>
