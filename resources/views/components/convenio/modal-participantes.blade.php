    ...
    <div class="space-y-6">
        ...
        @if($detalleSeleccionado['persona'] === 'moral')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-neutral-300">
                ...
                <p><span class="font-semibold text-neutral-100">RFC:</span> {{ $detalleSeleccionado['rfc'] ?? '-' }}</p>
                <p><span class="font-semibold text-neutral-100">Instrumento notarial:</span> {{ $detalleSeleccionado['instrumento'] ?? '-' }}</p>
                <p><span class="font-semibold text-neutral-100">Fecha del instrumento:</span> {{ $detalleSeleccionado['fecha_instrumento'] ?? '-' }}</p>
                <p><span class="font-semibold text-neutral-100">Teléfono:</span> {{ $detalleSeleccionado['telefono'] ?? '-' }}</p>
            </div>
        @endif
        ...
        @if($detalleSeleccionado['persona'] === 'fisica')
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-neutral-300">
                ...
                <p><span class="font-semibold text-neutral-100">Fecha de nacimiento:</span> {{ $detalleSeleccionado['fecha_nacimiento'] ?? '-' }}</p>
                <p><span class="font-semibold text-neutral-100">Sexo:</span> {{ $detalleSeleccionado['sexo'] ?? '-' }}</p>
                <p><span class="font-semibold text-neutral-100">Escolaridad:</span> {{ $detalleSeleccionado['escolaridad'] ?? '-' }}</p>
                <p><span class="font-semibold text-neutral-100">Ocupación:</span> {{ $detalleSeleccionado['ocupacion'] ?? '-' }}</p>
