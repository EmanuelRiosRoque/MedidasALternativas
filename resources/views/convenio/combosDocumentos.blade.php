...
@if ($materia === "mercantil")    
    ...
    @if (!empty($documentosCargados))
        ...
        <livewire:dropzone wire:model="archivosSubidos.{{ $index }}" :rules="['mimes:pdf', 'max:10240']" :multiple="false"
            :key="$doc . '-' . $index" />
...
@if (!empty($documentosFamiliaresCargados))
    @foreach($documentosFamiliaresCargados as $index => $doc)
        <label class="block text-sm font-medium text-zinc-700 mt-4">{{ $doc }}</label>
        <livewire:dropzone wire:model="archivosFamiliaresSubidos.{{ $index }}" :rules="['mimes:pdf','max:10240']" :multiple="false" :key="$doc . '-' . $index" />
    @endforeach
@endif
@endif
